...
/* ─────────────────────────────────────────────────────
   Types (mirrors what /stats/demo/full returns)
───────────────────────────────────────────────────── */
interface Overview {
  totalTrips: number;
...
interface FullDemoStats {
  overview: Overview;
  byTransport: TransportStat[];
  countries: Country[];
...
export default function RecapDemoPage() {
  ...
  const [data, setData]       = useState<FullDemoStats | null>(null);

  useEffect(() => {
    apiClient.get('/stats/demo/full')
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg, #f7f4ec 0%, #efe7da 100%)', fontFamily: 'Georgia, serif' }}>
      <div style={{ textAlign: 'center', color: '#9b7a4d' }}>
        ...
        <p style={{ fontSize: 18, margin: 0 }}>Loading demo recap…</p>
      </div>
    </div>
  );
  ...
  if (!data) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(180deg, #f7f4ec 0%, #efe7da 100%)', fontFamily: 'Georgia, serif' }}>
      <div style={{ textAlign: 'center', color: '#9b7a4d' }}>
        <p>Could not load demo data. <Link href="/map-demo" style={{ color: '#b5792a' }}>Back to map</Link></p>
      </div>
    </div>
  );

  const { overview, byTransport, countries, byYear, repeatedRoutes, carbonBreakdown: carbon } = data;
  ...
  const avgCarbonPerKm   = overview.totalDistanceKm > 0
    ? (overview.totalCarbonKg / overview.totalDistanceKm).toFixed(2) : '0.00';
  ...
  let personality = { title: 'World Wanderer', desc: 'This traveler explores the world in their own unique way.', emoji: '🌍' };
  if (flightPct > 60)      personality = { title: 'Sky Nomad',           desc: 'The skies are their second home — most journeys happen at 35,000 feet.',     emoji: '✈️' };
  else if (trainPct > 40)  personality = { title: 'Rail Adventurer',     desc: 'They prefer the scenic route — trains define their travel identity.',         emoji: '🚂' };
  else if (carPct > 40)    personality = { title: 'Road Tripper',         desc: 'The open road calls. Most at home behind the wheel.',                         emoji: '🚗' };
  else if (ferryPct > 20)  personality = { title: 'Sea Voyager',          desc: "A sailor's spirit — the sea weaves through this story.",                      emoji: '⛴️' };
  else if (byTransport.length >= 4) personality = { title: 'Multi-Modal Explorer', desc: 'No single mode defines them — they use every form of transport.', emoji: '🗺️' };
  ...
  const funFacts = [
    ...
    ...(ecoModes.length > 0
      ? [{ icon: '♻️', stat: `${ecoModes[0].cpk.toFixed(2)} kg/km`, label: `Greenest: ${tm(ecoModes[0].mode).label}`, sub: 'lowest CO₂ per km used' }] : []),
  ];
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '20px 24px 72px' }}>
        ...
        <div style={{ ...cardBase, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 28px', marginBottom: 28, backdropFilter: 'blur(4px)', background: 'rgba(255,255,255,0.55)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <div>
              <h1 style={{ margin: 0, fontSize: 24, color: '#5b3926' }}>Demo Travel Recap</h1>
              <p style={{ margin: 0, fontSize: 13, color: '#8b6a46', fontFamily: 'Arial, sans-serif' }}>
                A sample deep-dive — sign up to see your own
              </p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <Link href="/map-demo" style={{ color: '#7a5738', fontSize: 14, fontFamily: 'Arial, sans-serif', textDecoration: 'none', border: '1px solid rgba(170,140,95,0.28)', borderRadius: 999, padding: '8px 18px' }}>
              ← Map
            </Link>
            <Link href="/auth/signup" style={{ color: '#f7f2e8', fontSize: 14, fontFamily: 'Arial, sans-serif', textDecoration: 'none', background: 'linear-gradient(180deg, #b5792a 0%, #8d5a1f 100%)', borderRadius: 999, padding: '8px 18px' }}>
              Start Tracking
            </Link>
          </div>
        </div>
        ...
        {/* Demo banner */}
        <div style={{ background: 'rgba(181,121,42,0.1)', border: '1px solid rgba(181,121,42,0.25)', borderRadius: 16, padding: '13px 20px', marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
          <span style={{ fontSize: 14, color: '#7a5738', fontFamily: 'Arial, sans-serif' }}>
            👀 You're viewing the <strong>demo account's</strong> recap. Sign up to generate your own personal travel story.
          </span>
          <Link href="/auth/signup" style={{ fontSize: 13, color: '#b5792a', fontFamily: 'Arial, sans-serif', textDecoration: 'none', fontWeight: 600 }}>
            Create free account →
          </Link>
        </div>
          ...
          {overview.totalDistanceKm > 0 && (
            <div style={{ padding: '16px 20px', background: 'rgba(255,255,255,0.5)', borderRadius: 16, border: '1px solid rgba(190,175,145,0.2)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 13, color: '#b5792a', fontFamily: 'Arial, sans-serif', fontWeight: 600 }}>{moonPercent.toFixed(2)}%</span>
              </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, marginBottom: 24, alignItems: 'start' }}>
          <div style={{ ...cardBase, padding: '32px 36px' }}>
            <SectionTitle>How They Move</SectionTitle>
            {byTransport.length === 0 ? (
              <p style={{ color: '#9b7a4d', fontFamily: 'Arial, sans-serif' }}>No transport data.</p>
            ) : byTransport.map(t => {
            ...
            {ecoModes.length > 0 && (
              <div style={{ marginTop: 12, padding: '13px 16px', background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.18)', borderRadius: 14 }}>
                <span style={{ fontSize: 13, color: '#166534', fontFamily: 'Arial, sans-serif' }}>
                  🌱 Most eco-friendly mode: <strong>{tm(ecoModes[0].mode).emoji} {tm(ecoModes[0].mode).label}</strong> at {ecoModes[0].cpk.toFixed(2)} kg CO₂/km
                </span>
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {mostVisited && (
              <div style={{ ...cardBase, padding: '22px 22px' }}>
                <p style={{ margin: '0 0 8px 0', fontSize: 10, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#9b7a4d', fontFamily: 'Arial, sans-serif' }}>Most Visited Country</p>
            ...
            {busiestYear && (
              <div style={{ ...cardBase, padding: '22px 22px' }}>
                <p style={{ margin: '0 0 8px 0', fontSize: 10, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#9b7a4d', fontFamily: 'Arial, sans-serif' }}>Busiest Year</p>
                <div style={{ fontSize: 42, color: '#b5792a', lineHeight: 1, marginBottom: 6 }}>{busiestYear.year}</div>
                <div style={{ fontSize: 13, color: '#8b6a46', fontFamily: 'Arial, sans-serif' }}>{busiestYear.trips} trips · {fmt(busiestYear.distanceKm)} km</div>
                <div style={{ fontSize: 11, color: '#b5a090', fontFamily: 'Arial, sans-serif', marginTop: 4 }}>{busiestYear.countries} countries · {fmt(busiestYear.carbonKg)} kg CO₂</div>
              </div>
            )}
            <div style={{ ...cardBase, padding: '22px 22px' }}>
              <p style={{ margin: '0 0 8px 0', fontSize: 10, textTransform: 'uppercase', letterSpacing: '1.5px', color: '#9b7a4d', fontFamily: 'Arial, sans-serif' }}>Average Trip</p>
              <div style={{ fontSize: 32, color: '#5b3926', lineHeight: 1, marginBottom: 6 }}>{fmt(avgDistPerTrip)} <span style={{ fontSize: 14 }}>km</span></div>
        ...
        <div style={{ ...cardBase, padding: '32px 36px', marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, marginBottom: 28 }}>
            <SectionTitle>Countries Explored</SectionTitle>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(195px, 1fr))', gap: 12 }}>
            {countries.map((c, i) => (
              <div key={c.countryCode} style={{
                background: i === 0 ? 'linear-gradient(135deg, rgba(181,121,42,0.14), rgba(212,172,104,0.08))' : 'rgba(255,255,255,0.6)',
                border: `1px solid ${i === 0 ? 'rgba(181,121,42,0.28)' : 'rgba(190,175,145,0.2)'}`,
                borderRadius: 16, padding: '14px 15px', display: 'flex', alignItems: 'flex-start', gap: 10,
              }}>
                <span style={{ fontSize: 26, lineHeight: 1, flexShrink: 0, marginTop: 2 }}>{flagEmoji(c.countryCode)}</span>
                <div style={{ minWidth: 0, flex: 1 }}>
                  <div style={{ fontSize: 13, color: '#5b3926', fontWeight: i === 0 ? 600 : 400, marginBottom: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.countryName}</div>
                  <div style={{ fontSize: 11, color: '#9b7a4d', fontFamily: 'Arial, sans-serif' }}>{c.visitCount} {c.visitCount === 1 ? 'visit' : 'visits'} · {c.cities.length} {c.cities.length === 1 ? 'city' : 'cities'}</div>
                  <div style={{ fontSize: 10, color: '#b5a090', fontFamily: 'Arial, sans-serif', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.cities.slice(0, 3).join(', ')}{c.cities.length > 3 ? '…' : ''}</div>
                </div>
              </div>
            ))}
          </div>
                  ...
                  <div key={y.year} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 18, marginBottom: 10 }}>
                      <div style={{ minWidth: 56 }}>
                        ...
                        {isBusiest && <div style={{ fontSize: 9, color: '#b5792a', textTransform: 'uppercase', letterSpacing: '1px', fontFamily: 'Arial, sans-serif', marginTop: 2 }}>busiest</div>}
                      </div>
                      <div style={{ flex: 1 }}>
                      ...
                      <div style={{ minWidth: 110, textAlign: 'right', fontSize: 15, color: '#5b3926', fontFamily: 'Arial, sans-serif', fontWeight: 600 }}>{fmt(y.distanceKm)} km</div>
                    </div>
        ...
        {carbon && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, marginBottom: 24 }}>
            <div style={{ ...cardBase, padding: '32px 36px', background: 'linear-gradient(135deg, rgba(240,253,244,0.92), rgba(220,252,231,0.75))' }}>
              ...
              <div style={{ fontSize: 54, color: '#166534', lineHeight: 1, fontWeight: 600, marginBottom: 4 }}>{fmt(Math.round(carbon.totalCarbonKg))}</div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                <div style={{ background: 'rgba(255,255,255,0.62)', borderRadius: 16, padding: '18px 16px', border: '1px solid rgba(34,197,94,0.15)' }}>
                  ...
                  <div style={{ fontSize: 11, color: '#4d7c0f', fontFamily: 'Arial, sans-serif', marginTop: 4, lineHeight: 1.4 }}>trees absorbing CO₂ for a full year to offset this</div>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.62)', borderRadius: 16, padding: '18px 16px', border: '1px solid rgba(34,197,94,0.15)' }}>
                  <div style={{ fontSize: 30, marginBottom: 4 }}>🚗</div>
                  <div style={{ fontSize: 26, color: '#166534', fontWeight: 600 }}>{fmt(carbon.comparison.equivalentCarMiles)}</div>
                  <div style={{ fontSize: 11, color: '#4d7c0f', fontFamily: 'Arial, sans-serif', marginTop: 4, lineHeight: 1.4 }}>equivalent car miles at avg US emissions</div>
                </div>
              </div>
            </div>
            <div style={{ ...cardBase, padding: '32px 36px' }}>
                ...
                return (
                  <div key={t.mode} style={{ marginBottom: 18 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                      <span style={{ fontSize: 14, color: '#5b3926', fontFamily: 'Arial, sans-serif' }}>{meta.emoji} {meta.label}</span>
                      <div style={{ textAlign: 'right' }}>
        ...
        {repeatedRoutes.length > 0 && (
          <div style={{ ...cardBase, padding: '32px 36px', marginBottom: 24 }}>
            <SectionTitle>Signature Routes</SectionTitle>
                ...
                <div key={i} style={{ background: 'rgba(255,255,255,0.65)', border: '1px solid rgba(190,175,145,0.22)', borderRadius: 18, padding: '18px 20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                    {r.transportModes.map(m => (
                      <span key={m} style={{ fontSize: 11, background: 'rgba(181,121,42,0.1)', color: '#7a5738', borderRadius: 999, padding: '2px 10px', fontFamily: 'Arial, sans-serif', border: '1px solid rgba(181,121,42,0.18)' }}>
                  ...
                  <div style={{ fontSize: 11, color: '#b5a090', fontFamily: 'Arial, sans-serif', marginTop: 5 }}>avg {fmt(r.avgCarbonPerTrip)} kg CO₂ per journey</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ textAlign: 'center' }}>
          <p style={{ color: '#9b7a4d', fontFamily: 'Arial, sans-serif', fontSize: 15, marginBottom: 18 }}>
            Want a recap like this for your own travels?
          </p>
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href="/auth/signup" style={{ background: 'linear-gradient(180deg, #b5792a 0%, #8d5a1f 100%)', color: '#f7f2e8', textDecoration: 'none', borderRadius: 14, padding: '13px 30px', fontSize: 15, boxShadow: '0 6px 16px rgba(80,50,20,0.14)' }}>
              Create Free Account
            </Link>
            <Link href="/map-demo" style={{ background: 'rgba(255,255,255,0.78)', color: '#6b4b2f', textDecoration: 'none', borderRadius: 14, padding: '13px 30px', fontSize: 15, border: '1px solid rgba(180,150,100,0.25)' }}>
              Back to Demo Map
            </Link>
          </div>
        </div>
